...
![20211027165345.png](https://s2.loli.net/2025/05/28/5o6lvPCFT4gp2QH.png)
...
[线程的三种创建方式练习](https://gitee.com/zztiyjw/concurrent-practice/blob/master/src/main/java/com/albert/concurrent/basic/create)
...
1. `volatile` 变量可保证可见性，但不保证原子性。

   - `volatile` 修饰变量时，会把该线程本地内存中的该变量刷新到主内存中。

   - 同时让其它线程本地缓存里面该变量失效，这样就会读取主内存里面的最新数据。
     - 立即刷新回主内存+失效处理。

2. `volatile` 变量会禁止指令重排。保证有序性。

#### 原理-读写屏障

![image-20250528162546314](https://s2.loli.net/2025/05/28/ELOjPAbyvpRkcUd.png)

> 运行中的线程不是直接读取主内存中的变量，而是只能操作自己工作内存的变量。然后同步到主内存，线程之间的工作内存不共享。

1. 阻止屏障两边的指令**重排序**。
2. 写数据时加入屏障，强制将**线程私有工作内存的数据刷回主物理内存。**
3. 读数据时加入屏障，线程私有工作内存的数据**失效**，重新到**主物理内存中获取最新数据。**
...
[死锁活锁和饥饿总结](https://albertyang0801.github.io/blog/java/concurrent/single/死锁活锁和饥饿.html)
...
![20210710230037](https://s2.loli.net/2025/05/28/hHlQb9OT8D74uAN.png)
...
[ForkJoinPool线程池的练习](https://gitee.com/zztiyjw/concurrent-practice/blob/master/src/test/java/com/albert/concurrent/book/chapterthree/CountTask_14.java)
...
![20210929172852](https://s2.loli.net/2025/05/28/fkz8l1qZHGNQacI.png)
...
![20210930102855](https://s2.loli.net/2025/05/28/JjqrlkunUCX4iPW.png)
